...
thetaA=45;                %Angle between the x-component and the ordinary axis
...
%% Electric field spatial proprieties     
Eo_0=Ex_0*exp(1i*phase)*cos(theta)+Ey_0*sin(theta);  % Initial condition of field among the ordinary axis
Ee_0=-Ex_0*exp(1i*phase)*sin(theta)+Ey_0*cos(theta); % Initial condition of field among the extraordinary axis
 
% Check with central frequency monochromatic component z is in nm
%assuming a half-waveplate behaviour
figure(2)
%  FREE SPACE PROPAGATION CHECK
% for z=-1000:1:0
% Ex=Ex_0*exp(-1i*k_c*z)*exp(1i*phase);
% Ey=Ey_0*exp(-1i*k_c*z);
% E=[Ex;Ey];
% plotv(real(E),'b');
% hold on
% plot(real(Ex),real(Ey),'vk','Linewidth',2)
% axis([-1 1 -1 1])
% pause(0.01)
% hold off
% end     

% CHECK WITH VECTOR PLOT
% for z=0:1:1000
% Eo=Eo_0*exp(-1i*k_c*z);
% Ee=Ee_0*exp(1i*pi/2000*z-1i*k_c*z);
% 
...
% plotv(abs(E),'b');
% hold on
% plot(abs(Ex),abs(Ey),'vk','Linewidth',2)
% axis([-1 1 -1 1])
% pause(0.01)
% hold off
% end
...
z=i*10000:i*10000+2*10^4;
Eo=Eo_0*exp(-1i*k_c*z);
Ee=Ee_0*exp( 1i*pi/(1*10^8)*z-1i*k_c*z);
Ex=cos(theta)*Eo-Ee*sin(theta);
Ey=sin(theta)*Eo+cos(theta)*Ee;


g1=plot3(z,real(Ex),real(Ey))
xlabel ("Space(nm)");
ylabel ("E_y ( arb.)");
zlabel ("E_x ( arb.)");
grid on
...
title('Half waveplate monochromatic field')
axis([z(1) z(end) -1 1 -1 1]) % caso 3D
pause(0.1)
end 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Propagation of a pulse in the anisotropic material
...
PHI=40*(f-freq_IR);%(ones(length(f),1)*pi)'; %Data taken from BhalleWaveplate 1) HWP 2) QWP 3) Retarder
...
Ex_t=real(fftshift(ifft((Ex))));
Ey_t=real(fftshift(ifft((Ey))));

Ex_0t=real(fftshift(ifft((Ex_f0))));
Ey_0t=real(fftshift(ifft((Ey_f0))));
...
PHI=(ones(length(f),1)*pi)'; %Data taken from BhalleWaveplate 1) HWP 2) QWP 3) Retarder
z0=2*10^4; %Thickness of the object

z=2*10^4; %20 um thick object
delta=PHI/z0;
